          ...
          if (_this.ruleForm.verificationCode) {
            if (Number(_this.ruleForm.verificationCode) != _this.yzms) {
              ...
              instance({
                ...
                method: "post",
                url: this.urlb + "/api3/UserLogin/userLogin",
                // data: Qs.stringify({
                ...
                if (res.data.code == 107) {
                  setTimeout(function () {
                    ...
                    localStorage.setItem(res.data.data.username + "uid", res.data.data.uid); // 用户id：用户的唯一标识
                    localStorage.setItem("gid", res.data.data.group_id); // 1、企业用户；2、个人用户；3、定制用户
                    localStorage.setItem("role", res.data.data.role); // 1、企业用户；2、个人用户；3、定制用户
                  ...
                  _this.ghyzm();
                } else if (res.data.code == 151) {
                  ElementUI.Loading.service({}).close();
                  _this.$message.warning("子管理员不能登录网页");
                  _this.ghyzm();
                } else if (res.data.code == 105) {
                  ElementUI.Loading.service({}).close();
                  _this.$message.warning("此账号已被限制登录");
                  _this.ghyzm();
                } else {
                  ElementUI.Loading.service({}).close();
