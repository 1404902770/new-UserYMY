...
export default {
  ...
  methods: {
    ...
    getEqFaZhi() {
      ...
      let uid = localStorage.getItem(username + 'uid');
      let type = "post";
      let url = this.urla + "/api3/device/getMachedVal";
      let data = {
        data: {
    ...
    submitForm1() {
      if (this.form.selects1 && this.form.selects2) {
        ...
        let uid = localStorage.getItem(username + 'uid');
        let type = "post";
        let url = this.urlb + "/api3/device/getMachedVal";
        let data = {
          data: {
            uid: uid,
            // uid: localStorage.getItem("uid"),
    ...
    submitForm2(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          let uid = localStorage.getItem(username + 'uid');
          let type = "post";
          let url = this.urla + "/api3/device/setYuzhiVal";
          let data = {
            data: {
              uid: uid,
              // uid: localStorage.getItem("uid"),
          ...
          data.data.vl1 = this.ruleForm.vl1; // 欠压预警值
          data.data.tl1 = this.ruleForm.tl1; // 欠压预警持续时间
          data.data.vh1 = this.ruleForm.vh1; // 过压预警
          data.data.th1 = this.ruleForm.th1; // 过压预警时间
          data.data.vh2 = this.ruleForm.vh2; // 过压报
          data.data.th2 = this.ruleForm.th2; // 过压报警时间
          data.data.dvk = this.ruleForm.dvk; // 过压报警超过设定时间的动作
          data.data.cl1 = this.ruleForm.cl1; // 电流预警
          data.data.t1 = this.ruleForm.t1; // 电流预警时间
          data.data.al = this.ruleForm.al; // 预警动作
          data.data.cl2 = this.ruleForm.cl2; // 电流告警
          data.data.t2 = this.ruleForm.t2; // 告警时间
          data.data.a2 = this.ruleForm.a2; // 告警动作
          data.data.cl3 = this.ruleForm.cl3; // 电流报警
          data.data.t3 = this.ruleForm.t3; // 报警时间
          data.data.a3 = this.ruleForm.a3; // 报警动作
          data.data.l1 = this.ruleForm.l1; // 漏电流
          data.data.tl2 = this.ruleForm.tl2; // 漏电流持续时间
          data.data.dz = this.ruleForm.dz; // 漏电流动作
          data.data.wendu = this.ruleForm.wendu; // 温度
          data.data.wendut = this.ruleForm.wendut; // 温度持续时间
          data.data.wendul = this.ruleForm.wendul; // 温度动
          this.myAjax(type, url, data, res => {
            // console.log(res)
    ...
    setThreshold(instruction) {
      ...
      let data = {
        data: {
          ...
          // uid: localStorage.getItem("uid"),
          nid: this.form.selects1,
          mid: this.form.selects2, //线路号
          type: 'web', //类型
          lon: 0,//经度
          lat: 0,//纬度
          device: "03" + instruction,//操作命令，见下表说明
        }
      };
      this.myAjax(type, url, data, res => {
        // console.log(res)
...
.thresholdSet {
  ...
  .el-main {
    ...
    .el-form {
      ...
      padding-left: 10px;
      .el-form-item /deep/ .el-form-item__label,
      .el-form-item /deep/ .el-radio__label {
        color: #ccc !important;
      }
