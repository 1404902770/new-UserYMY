...
export default {
  ...
  methods: {
    ...
    getxianlu() {
      ...
      let uid = localStorage.getItem(username + 'uid');
      let type = 'post'
      let url = this.urla + '/api3/EnergyData/machedKvWave'
      let data = {
        data: {
          uid: uid,
          // uid: localStorage.getItem('uid'),
      ...
      this.myAjax(type, url, data, res => {
        ...
        this.tableData.forEach(val => {
          if (val.name == "kv1") {
            val.name = 'A相'
          } else if (val.name == "kv2") {
            val.name = 'B相'
          } else if (val.name == "kv3") {
            val.name = 'C相'
          }
        })

        if (res.data.data.kv1) {
          this.linedata = res.data.data.kv1
        } else {
          this.linedata = res.data.data.kv
        }

        if (res.data.data.kv2) {
          this.linedata2 = res.data.data.kv2
        } else {
          this.linedata2 = []
        }

        if (res.data.data.kv3) {
          this.linedata3 = res.data.data.kv3
        } else {
          this.linedata3 = []
        }
    ...
    getStudent1() {
      ...
      setTimeout(() => {
        ...
        _this.student.setOption({
          ...
          series: [
            {
              name: this.linedata2.length > 0 ? 'A相' : '无功功率',
              type: 'line',
              smooth: true,
              symbol: 'none',
              sampling: 'average',
            ...
            {
              name: this.linedata2.length > 0 ? 'B相' : '线路',
              type: 'line',
              smooth: true,
              symbol: 'none',
              sampling: 'average',
            ...
            {
              name: this.linedata3.length > 0 ? 'C相' : '线路',
              type: 'line',
              smooth: true,
              symbol: 'none',
              sampling: 'average',
